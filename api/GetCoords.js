async function fetchRouteData(originLatLong, destinationLatLong) {
  ...
  console.log("destinationLatLong: ", destinationLatLong);
  console.log("origin: ", origin, "destination: ", destination);
  try {
    const response = await fetch(
      `${proxyUrl}https://maps.googleapis.com/maps/api/directions/json?origin=${origin}&destination=${destination}&key=${APIKEY}`
    );
